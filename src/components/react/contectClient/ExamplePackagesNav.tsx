...
import { EventBus } from "../../utils/eventBus";  // import estático arriba del todo
...
export default function ExamplePackagesNav({ currentOrderId }: ExamplePackagesNavProps) {
    ...
    const handleOpenModal = async () => {
        window.dispatchEvent(new CustomEvent("openSongModal"));94
    };
                ...
                `}
                            ...
                            >
                                ...
                                <div className="flex flex-col min-w-0">
                                    <span
                                        className={`text-sm font-bold leading-tight ${isCurrent ? "text-white" : "text-[#0E5050]"
                                            }`}
                                    >
                                        {pkg.label}
                                        {isCurrent && (
                                    ...
                                    <span
                                        className={`text-xs leading-tight mt-0.5 truncate ${isCurrent ? "text-white/70" : "text-gray-400"
                                            }`}
                                    >
                                        {pkg.description}
                                    </span>
            ...
            <div className="flex justify-center mt-4">
                <button
                   onClick={handleOpenModal}
                    className=" group cursor-pointer inline-flex items-center justify-center   font-bold transition-all hover:scale-105 active:scale-95 rounded bg-gradient-to-r from-[#f89a3f] via-[#ff6b35] to-[#f89a3f] bg-[length:200%_auto] hover:bg-right text-white  px-8 py-3 "
                >
                    <svg
                        ...
                        className="lucide lucide-music mr-2 h-5 w-5 group-hover:rotate-12 transition-transform"
